...
前两项分别是16项和6项保存文件的虚拟控制，parser文件是9项fake transport控制；它们都不加载模型。base analyzer没有内置self-test，wrapper没有可安全导入的测试入口。公开base CLI增加验收检查，原库函数`analyze()`的对账/schema/计时口径保持不变；该文件的SHA已不同于C3原脚本。CPU通过不能代替真实GPU、服务生命周期或新二进制上的长时验收。
