...
这份历史记录保留了原 README 中的局部 Core ML / ANE 结果、运行命令、来源及后续 GPU 转型说明。文中的“当前”“新进展”“下一步”属于当时的实验快照；当前工程能力、默认设置与运行入口以 [README](../README.md) 为准。

这些实验没有建立整模型 ANE 推理或相对 MLX GPU 的性能优势。`results/`、模型包和原始 trace 是本机产物，不随 Git 克隆提供；旧命令中的父工作区 uv 路径和预制模型包也不构成独立克隆后的安装步骤。下文所有相对链接已按本文件位于 `docs/` 调整，命令中的路径仍保留原运行位置。
