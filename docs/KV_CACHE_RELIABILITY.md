...
输出检查的边界：事后核对10个oracle均为16-token输出，且全部具有同一个文本SHA。它们对应的输入前缀与归档工作集确实不同，但短输出不能充分区分跨前缀误恢复。本轮支持持续生命周期、资源及既定文本一致性检查，不能独立证明任意会话隔离；混合状态数值正确性需结合下文单独的完整张量对照。后续发布配置长测应使用可区分的预期输出，或在恢复时加入状态对照，不能仅增加相同短输出的请求数。
