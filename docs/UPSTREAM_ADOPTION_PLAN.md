...
2026-09-09 更新执行顺序。目标仍是让当前 Qwen3.8-Flash-Next checkpoint 在 M5 Max 上稳定、快速运行，保留 Swift 独立 runner；项目名 Qwen4-MLX 不表示更换模型。本页区分已实现基础、正在验证的候选和后续工作，不把上游功能清单当成本项目的支持列表。

当前 KV cache 的统一能力定义、优先级、依赖及工业发布门槛以[关键能力计划](KV_CACHE_CAPABILITIES.md)为准。新增核对 vLLM、SGLang、LMCache、DwarfStar 与 MLX LM 的当前固定快照；完整会话复用、系统压力联动、SSD 成本调度、真实页共享等均区分已有基础与缺口。最初2026-09-09的研究提交只更新计划与证据链接；后续实际落地以能力计划的分版本记录为准。

用户最新决定：**MTP 性能优化放到整体计划后段。** 先推进 AR 服务与请求生命周期、完整前缀状态复用、缓存额度/淘汰和 SSD 状态缓存，并分别优化 prefill / decode 基础路径。MTP 的性能或完整发布门槛不再阻塞这些工作；各项仍须通过自身的正确性、资源与生命周期检查。现有显式 MTP 保留并维护必要回归，新 kernel、深度调优和收益窗口延后。单独缓存 Attention KV 不足以恢复这个混合模型。

## 早期来源与适配边界

下表保留先前研究快照，不表示 2026-09-09 最新版本；本次 KV 专项的固定提交与具体机制见[来源矩阵](KV_CACHE_CAPABILITIES.md#三从上游吸收什么)及其三份源码评审。
...
这些基础支持下面的局部实验；HTTP 工具闭环及前缀树的本轮验收见 [AR 缓存与服务](AR_PREFIX_CACHE.md)，跨进程 PD、连续批处理仍未实现；SSD 状态缓存和联合额度的后续实现及验证见 [缓存可靠性](KV_CACHE_RELIABILITY.md)。此前 MoE 组合单层 +5.21%，完整 prefill 828→824 token/s，已保留为可选，并未因此替换默认。

## 实施顺序（2026-09-09确定，能力状态持续更新）

| 优先级 | 工作 | 交付与验收重点 |
| --- | --- | --- |
| 已有基线 | AR 工具服务、完整混合状态快照、radix/LRU、联合额度、single-flight 与可选持久化 SSD | 具体版本、短窗压力、重启及故障结果见[缓存可靠性](KV_CACHE_RELIABILITY.md)；HTTP已自动查找完整会话/工具历史的准确前缀；M2物理KV页附件已显式接入库和HTTP，仍保留dense快照，K07整项与工业验收未完成 |
| A，当前重点 | K01/03/04/06：恢复所有权、真实内存压力、SSD 最小空间/有界恢复等待/读写控制、有效 token 与成本指标 | 在现有完整快照基础上补故障与压力行为，继续保持准入、状态完整性与取消清理有界 |
| B，与 A 持续验收 | K02/05：已接入完整多轮/工具结果/分支复用，继续公平的本机PD调度验收 | 保留chunk416数值边界与有限检查点；完整自动复用已落地不等于A/B组合的P0发布门槛已通过 |
| C，容量与基础性能 | K07：Attention/QSA 真正分页、共享完整页与尾页 COW | 直接页表读写，减少整段 concat/复制；GDN/PLE 保留准确检查点。分别验收 prefill、decode、内存与数值 |
| D，成本优化 | K08/09：SSD 增量分块、引用回收及按价值保留 | 成本 trace/shadow 可提前进行；增量存储依赖稳定页布局。有可重复收益才替换默认 LRU/存储策略 |
| E，核心成熟后 | 活跃请求暂停卸载/恢复，再评估远端 PD、压缩及最终 MTP 配合 | 执行快照另含阶段与输出状态；前缀 archive 不等于可恢复请求。MTP 性能继续后置 |

前述核心缓存工作不等待 MTP 收益成立，也不要求先完成全套 MTP 发布检查。首版 AR 缓存遇到 MTP 请求仍走原 MTP 冷 prefill，不将请求悄悄改成 AR。任何共享状态或生命周期改动影响已有 MTP 路径时，做必要正确性回归；这不等于重新启动 MTP 性能优化。缓存不得发布半份状态，错误后的资源清理和请求隔离始终是当前功能本身的验收项。

## 既有工作记录（原阶段编号，仅用于追溯）

| 原编号 / 状态 | 工作 | 最小落点 | 已保存的范围与结果 |
| --- | --- | --- | --- |
...
上述原编号保留历史阶段归属，不再代表下一步优先级。后续代码工作可并行，GPU 实测仍串行。微测平或更慢就停止扩大该候选；完整模型没有可重复收益就维持现有默认。初步以局部约 5%、整模型约 3% 作为值得继续的筛选量级，最终决策结合运行漂移，不能因一次跨过阈值宣布成功。

[本机调度优先级补充](research/LOCAL_SCHEDULER_NEXT.md)进一步核对了当前接口：多会话FIFO交替并不合并跨请求权重读取，burst计步骤而非GPU时间，固定prefill块仍会阻塞其他decode。MTP[两窗口性能复测](MTP_RELEASE_WINDOWS.md)已完成：36组中23通过、13漂移未定，未通过发布性能门槛；这个结果不再阻塞 AR 精确 checkpoint。[确定到达时点的PD实验](PD_DECODE_ARRIVAL_EXPERIMENT.md)已完成13请求正确性回归：短请求等待缩短，但首对整体时间/长TTFT超限且外基线漂移明显，默认burst4保持。真正跨请求batching需要改造独立位置及混合状态接口，暂不排在前缀复用之前。
...
阶段时间只在定义互斥时求和。未覆盖的 host 时间、回调/排队时间单独列出，不给未知值编造分布。后段的自适应深度工作要以整轮成本与等待控制，并保持 head history；暂停 MTP 后重新启用需要明确同步规则。[现有 MTP 发布条件](MTP_RELEASE_CRITERIA.md)用于 MTP 自身验收，不作为前述 AR 缓存和服务工作的前置条件。
...
[主线整合与候选状态](MAINLINE_INTEGRATION.md)记录分支成果的归属。阶段结束时将可用代码、复现命令和默认决策及时纳入 `codex/runner-baseline`；默认启用与代码合入分别判断，负结果也保留明确处置，避免成果只留在实验分支。

[实际验证专家重叠](MTP_EXPERT_OVERLAP.md)已完成原11k/128诊断：55个S3验证轮次覆盖全部48层，26.7184%的专家选择在同轮同层重复。仅gate/up共享的理想节省为全部路由权重逻辑量的17.8123%。[首个共享读取原型](MTP_GROUPED_GATEUP_EXPERIMENT.md)288项逐位检查通过，但六组没有建立胜过当前逐token路径的可重复收益，保留显式探针、不接生成。现有AR/D2普通入口随后通过完整11k/128回归；尚无实际DRAM节省或候选整模型收益结论，原默认和MTP发布门槛保持。
