...
| 服务交付线，实验入口与短soak通过 | [HTTP/SSE、背压和断连](HTTP_SERVER_EXPERIMENT.md) | 固定推理线程、独立网络队列、有界输出及text-only协议；29项CPU、19+15项live及12周期46项检查 | AR/MTP响应、断连恢复、限额与接收期限通过；13分29秒短soak中FD稳定，RSS增1.52MiB，不外推长期无泄漏；其他期限仍待测 |
...
最新[供电环境检查](POWER_ENVIRONMENT_FINDINGS.md)发现当前协商40 W，三个离散AC快照电量100%/73%/45%；这不证明漂移原因。先完成服务稳定性和正确性，供电路径确认后再追加性能对照，保留所有旧分类和默认值。
