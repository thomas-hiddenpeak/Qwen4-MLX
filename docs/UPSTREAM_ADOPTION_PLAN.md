...
2026-09-08 更新执行顺序。目标仍是让当前 Qwen3.8-Flash-Next checkpoint 在 M5 Max 上稳定、快速运行，保留 Swift 独立 runner；项目名 Qwen4-MLX 不表示更换模型。本页区分已实现基础、正在验证的候选和后续工作，不把上游功能清单当成本项目的支持列表。
...
| 3，当前重点 | SSD 状态 offload / 恢复、联合容量与并发缓存生命周期 | 已实现可选持久化、校验/TTL/LRU、request/cache/workspace 联合额度与同前缀合并；按[缓存可靠性](KV_CACHE_RELIABILITY.md)逐项记录重启、损坏、取消、压力与持续运行结果。MTP 性能继续后置 |
...
前四项不等待 MTP 收益成立，也不要求先完成全套 MTP 发布检查。首版 AR 缓存遇到 MTP 请求仍走原 MTP 冷 prefill，不将请求悄悄改成 AR。任何共享状态或生命周期改动影响已有 MTP 路径时，做必要正确性回归；这不等于重新启动 MTP 性能优化。缓存不得发布半份状态，错误后的资源清理和请求隔离始终是当前功能本身的验收项。
