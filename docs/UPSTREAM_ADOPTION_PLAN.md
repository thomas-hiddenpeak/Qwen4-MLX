...
当前 KV cache 的统一能力定义、优先级、依赖及工业发布门槛以[关键能力计划](KV_CACHE_CAPABILITIES.md)为准。新增核对 vLLM、SGLang、LMCache、DwarfStar 与 MLX LM 的当前固定快照；完整会话复用、系统压力联动、SSD 成本调度、真实页共享等均区分已有基础与缺口。本轮只更新计划与证据链接，未修改运行代码。
...
| 优先级 | 工作 | 交付与验收重点 |
| --- | --- | --- |
| 已有基线 | AR 工具服务、完整混合状态快照、radix/LRU、联合额度、single-flight 与可选持久化 SSD | 具体版本、短窗压力、重启及故障结果见[缓存可靠性](KV_CACHE_RELIABILITY.md)；HTTP 自动复用当前限系统/工具前缀，未完成物理分页 |
| A，当前重点 | K01/03/04/06：恢复所有权、真实内存压力、SSD 最小空间/有界恢复等待/读写控制、有效 token 与成本指标 | 在现有完整快照基础上补故障与压力行为，继续保持准入、状态完整性与取消清理有界 |
| B，紧接 A | K02/05：完整多轮/工具结果/分支自动复用与公平的本机 PD 调度 | 保留 chunk416 的数值边界与有限检查点，减少实际重复 prefill；A/B 组合完成 P0 发布门槛 |
